...
export default function TweetCard({ tweet }: any) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && user) {
      const stored = localStorage.getItem(`twiller-bookmarks-${user._id}`);
      if (stored) {
        try {
          const bookmarksList = JSON.parse(stored);
          setIsBookmarked(bookmarksList.includes(tweetstate._id));
        } catch (e) {
          console.error(e);
        }
      }
  ...
  const toggleBookmark = (e: React.MouseEvent) => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (stored) {
        try {
          bookmarksList = JSON.parse(stored);
        } catch (e) {
          bookmarksList = [];
        }
      }

      if (bookmarksList.includes(tweetstate._id)) {
        bookmarksList = bookmarksList.filter((id: string) => id !== tweetstate._id);
        setIsBookmarked(false);
      } else {
        bookmarksList.push(tweetstate._id);
      ...
      <CardContent className="p-4">
        <div className="flex space-x-3">
          <Avatar className="h-12 w-12">
            ...
            />
            <AvatarFallback>{tweetstate.author?.displayName?.[0] || "U"}</AvatarFallback>
          </Avatar>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between max-w-md">
              ...
              >
                <Bookmark className={`h-5 w-5 ${isBookmarked ? "text-blue-500 fill-current" : "group-hover:text-blue-400"}`} />
              </Button>
